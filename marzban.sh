...
install_marzban_script() {
    FETCH_REPO="UtyaVPN/UtyaVPN-Marzban-scripts"
...
services:
  marzban:
    image: utyavpn/utyavpn-marzban:${marzban_version}
    restart: always
    env_file: .env
    network_mode: host
    volumes:
...
EOF
        ...
        if [ "$marzban_version" == "latest" ]; then
            yq -i '.services.marzban.image = "utyavpn/utyavpn-marzban:latest"' "$docker_file_path"
        else
            yq -i ".services.marzban.image = \"utyavpn/utyavpn-marzban:${marzban_version}\"" "$docker_file_path"
        fi
...
install_command() {
    ...
    check_version_exists() {
        local version=$1
        repo_url="https://api.github.com/repos/UtyaVPN/UtyaVPN-Marzban/releases"
    ...
    if [ -z "$ADMIN_USERNAME" ]; then
        ...
        if marzban_cli admin create --username "$ADMIN_USERNAME" --sudo; then
            ...
            colorized_echo red "Failed to create admin user '$ADMIN_USERNAME'. It might already exist or another error occurred."
        fi
    fi
